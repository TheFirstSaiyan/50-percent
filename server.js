  ...
  for (let id of Object.keys(paired)) {
    if (id == socketId)
      ...
      //sss  let timerFlag = 0;
        ...
        //ss function timerMessageSuccess() {
        //   timerFlag = 1;
        // }
      ...
      //sss  socket.emit("startTimer", "well", timerMessageSuccess);
        //s if (timerFlag == 0) {
        //s   socket.broadcast.to(socketId).emit("startTimer",
        //s     "well hello");
        //s }
      //sss  socket.broadcast.to(pairs[socketId]).emit("startTimer",
      //sss    "well...");
...
//sss  socket.on('checkRank',checkRank);
...
//writeStream.end();
  ...
  //sss  socket.emit('rankInfo',rank);
    //socket.emit.to()
  //sss  socket.broadcast.to(pairs[socket.id]).emit('rankInfo',rank);
     paired[pairs[socket.id]] = "occupied on replay";
     paired[socket.id]="occupied on replay";
    //
    // turns[pairs[socket.id]]=undefined;
  ...
  function checkGuess(guess) {
    ...
    if (guess.replace(" ", "") == corrects[socket.id].replace(" ", "") && turns[socket.id] ==
      ...
      //increase score
    //sss  socket.emit('increaseScore','good job');
    //sss  socket.broadcast.to(pairs[socket.id]).emit('increaseScore','good job');
      //clear screen
      socket.emit('clearInstruction',
        'clear your screen dood');
  ...
  //sss  socket.broadcast.to(pair).emit('clearTimer',
  //sss    'time starts now');
    pairs[socket.id] = undefined;
    turns[socket.id] = undefined;
